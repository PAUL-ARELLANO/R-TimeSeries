...
#              variations, linear trends, and random noise. The bark beetle event
#              is simulated as a progressive decline in NDVI starting from a specific
#              date. The script generates a CSV file for each combination of parameters
...
combinations <- expand.grid(
  noise = seq(from = 0.05, to = 0.15, by = 0.05),
  trend = seq(from = 0.2, to = -0.2, by = -0.2),
  post_event_decline = seq(from = -0.2, to = -0.4, by = -0.1),
  random_NA = seq(from = 0.10, to = 0.20, by = 0.05),
  amplitude = seq(from = 0.4, to = 0.55, by = 0.05),
  phase_shift = c(-0.05, -0.02, 0.02, 0.05)
)
...
for (i in 1:nrow(combinations)) {
  ...
  phase_shift <- combinations$phase_shift[i]
  ...
  # **Apply Random Amplitude Reduction for Spring & Summer**
  seasonal_reduction <- ifelse(month(dates) %in% 3:8, runif(n, min = 0.1, max = 0.2), 0)
  adjusted_amplitude <- amplitude * (1 - seasonal_reduction)

  # Compute Seasonal Cycle
  seasonal_cycle <- adjusted_amplitude * sin(2 * pi * ((1:n) / 365 + phase_shift))

  # Set a minimum offset
  offset <- runif(1, min = 0.001, max = 0.002)
  seasonal_cycle <- seasonal_cycle + (offset - min(seasonal_cycle))

  # Compute NDVI
  ndvi <- 0.3 * seasonal_cycle + trend + noise
  ndvi <- pmax(ndvi, offset)

  # Bark Beetle Event: NDVI Drop
  event_index <- which(dates >= ymd("2022-05-15"))
  decline <- rep(0, n)
  ...
  # Add Random NAs
  set.seed(42)
  na_indices <- sample(1:n, floor(na_percent * n))
  ndvi[na_indices] <- NA
  ...
  file_name <- paste0("NDVI_noise", noise_sd,
                      "_trend", trend_rate,
...
setwd("C:/Users/pa589/NAU/TREE_STRESS/TreeStress_detection/MODELS/Synthetic_datasets/Stresses_dataset_28-Apr-25") # Replace it to your working directory 
